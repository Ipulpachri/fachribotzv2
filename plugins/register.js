...
let handler = async function (m, { text, usedPrefix }) {
  ...
  if (!Reg.test(text)) throw `Format salah!\nContoh: *${usedPrefix}daftar ${conn.getName(m.sender)}.15*`
...
`.trim()
await conn.send2ButtonLoc(m.chat, await(await fetch(image)).buffer(), caption, watermark, 'Menu', `.menu`, 'Tutorial', `.tutorial`, m)
}
